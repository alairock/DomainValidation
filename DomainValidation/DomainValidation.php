 ...
 *
 * This has been updated by @alairock for packagist
 *
 */
 ...
 */
	public function validate($domain, $verify = false) {
		...
		if ($verify) {
			$httpCode = $this->curl->checkUrlStatus($domain);
			if (!$httpCode) {
				return false;
			}
		}
 ...
 **/
	public function urlParts($url = null) {
		if (!$url) {
			throw new \Exception('URL was not passed (urlParts).');
		}
		$parts = parse_url($url);
 ...
 * @throws Exception A url must be passed to addScheme
 **/
	public function hasScheme($url = null) {
		if (!$url) {
			throw new \Exception('URL was not passed (hasScheme).');
		}
 ...
 **/
	public function addScheme($url, $scheme = 'http') {
		if (!$url) {
			throw new \Exception('URL was not passed (addScheme).');
		}
 ...
 **/
	public function removeScheme($url = null) {
		if (!$url) {
			throw new \Exception('URL was not passed (removeScheme).');
		}
		if ($this->hasScheme($url)) {
			$parts = $this->urlParts($url);
 ...
 * @access public
 * @throws Exception A domain must be passed to addScheme
 **/
	public function domainOnly($url = null) {
		if (!$url) {
			throw new \Exception('You must pass the url.');
		}
		$parts = $this->urlParts($url);
		$username = '/';
 ...
 * @throws Exception A url must be passed
 **/
	public function getUsernameFromTempUrl($url = null) {
		if (!$url) {
			throw new \Exception('URL was not passed or is invalid (getUsernameFromTempUrl).');
		}
		if (!$this->isTemporaryUrl($url)) {
			return false;
		}
 ...
 **/
	public function hasWww($url = null) {
		if (!$url) {
			throw new \Exception('Domain was not passed (hasWww).');
		}
		$parts = $this->urlParts($url);
 ...
 **/
	public function addWww($url = null) {
		if (!$url) {
			throw new \Exception('URL was not passed or is invalid (addWww).');
		}
 ...
 * @access public
 * @throws Exception A url must be passed to
 **/
	public function isTemporaryUrl($url = null) {
		if (!$url) {
			throw new \Exception('URL was not passed (isTemporaryUrl).');
		}
		$parts = $this->urlParts($url);
 ...
 **/
	public function isSubdomain($url = null) {
		if (!$url) {
			throw new \Exception('URL was not passed (isSubdomain).');
		}
		if ($this->isIp($url)) {
			return false;
		}
